...
echo "use {$config->getFullyQualifiedName(false, PHPFHIR_ENUM_CONFIG_KEY)};\n";
echo "use {$config->getFullyQualifiedName(false, PHPFHIR_CLASSNAME_CONSTANTS)};\n";

?>

class <?php echo PHPFHIR_TEST_CLASSNAME_CONFIG; ?> extends TestCase
{
    ...
    {
        $data = [
            <?php echo PHPFHIR_ENUM_CONFIG_KEY; ?>::REGISTER_AUTOLOADER->value => true,
            <?php echo PHPFHIR_ENUM_CONFIG_KEY; ?>::LIBXML_OPTS->value => 12345,
            <?php echo PHPFHIR_ENUM_CONFIG_KEY; ?>::ROOT_XMLNS->value => 'xmlns://example.org',
            <?php echo PHPFHIR_ENUM_CONFIG_KEY; ?>::OVERRIDE_SOURCE_XMLNS->value => true,
        ];
        $config = new <?php echo PHPFHIR_CLASSNAME_CONFIG; ?>($data);
        foreach($data as $k => $v) {
