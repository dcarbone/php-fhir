...
use <?php echo $config->getFullyQualifiedName(false, PHPFHIR_CLIENT_ENUM_SERIALIZE_FORMAT); ?>;
...
class <?php echo $testClassname; ?> extends TestCase
{
    ...
    protected function fetchResourceBundle(string $format): string
    {
        ...
        $rc = $this->_client->readRaw(
            ...
            count: 5,
            format: <?php echo PHPFHIR_CLIENT_ENUM_SERIALIZE_FORMAT; ?>::from($format),
        );
        ...
        if ('' !== $dlDir) {
            ...
            $fname = sprintf('%s%s<?php echo $type->getFHIRName(); ?>-<?php echo $version->getSourceMetadata()->getSemanticVersion(false); ?>-source.%s', $dlDir, DIRECTORY_SEPARATOR, $format);
